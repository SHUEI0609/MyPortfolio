...
import { c as createComponent } from "./astro-component_y0qR-KYE.mjs";
import { k as renderComponent, r as renderTemplate, m as maybeRenderHead } from "./worker-entry_699v-LVJ.mjs";
import { $ as $$BaseLayout } from "./BaseLayout_CmXUaMkp.mjs";
...
LOGIN
</button> <p id="login-error" class="text-red-400 text-xs text-center hidden">パスワードが正しくありません</p> </form> </div> </div>  <div id="admin-dashboard" class="hidden min-h-screen bg-[#FAFAFA]"> <!-- Admin Header --> <header class="bg-black text-white px-4 sm:px-6 py-4 flex flex-col sm:flex-row gap-4 sm:gap-0 sm:justify-between sm:items-center sticky top-0 z-50"> <div class="flex items-center gap-3"> <div class="w-3 h-3 bg-white"></div> <span class="font-black tracking-tighter">SHUEI<span class="opacity-50">.CMS</span></span> </div> <div class="flex w-full sm:w-auto items-center justify-between sm:justify-end gap-4"> <a href="/" target="_blank" class="text-xs font-bold tracking-widest text-zinc-400 hover:text-white transition-colors flex items-center gap-1">
...
LOGOUT
</button> </div> </header> <!-- Tabs --> <div class="border-b border-zinc-200 bg-white"> <div class="admin-tabs-scroll max-w-5xl mx-auto px-4 sm:px-6 flex overflow-x-auto"> <button onclick="switchTab('history')" id="tab-history" class="admin-tab active px-4 sm:px-6 py-4 text-xs font-bold tracking-widest border-b-2 border-black transition-colors whitespace-nowrap shrink-0">
HISTORY
</button> <button onclick="switchTab('skills')" id="tab-skills" class="admin-tab px-4 sm:px-6 py-4 text-xs font-bold tracking-widest border-b-2 border-transparent text-zinc-400 hover:text-black transition-colors whitespace-nowrap shrink-0">
SKILLS
</button> <button onclick="switchTab('projects')" id="tab-projects" class="admin-tab px-4 sm:px-6 py-4 text-xs font-bold tracking-widest border-b-2 border-transparent text-zinc-400 hover:text-black transition-colors whitespace-nowrap shrink-0">
PROJECTS
</button> <button onclick="switchTab('topics')" id="tab-topics" class="admin-tab px-4 sm:px-6 py-4 text-xs font-bold tracking-widest border-b-2 border-transparent text-zinc-400 hover:text-black transition-colors whitespace-nowrap shrink-0">
TOPICS
...
キャンセル
</button> </div> </form> </div> </div>  <div id="delete-overlay" class="hidden fixed inset-0 bg-black/60 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4"> <div class="bg-white w-full sm:max-w-sm shadow-2xl p-6 text-center"> <div class="w-12 h-12 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-4"> <i data-lucide="trash-2" class="w-5 h-5 text-red-500"></i> </div> <h3 class="font-bold text-lg mb-2">削除の確認</h3> <p class="text-sm text-zinc-500 mb-6">この項目を削除しますか？この操作は取り消せません。</p> <div class="flex flex-col sm:flex-row gap-3"> <button onclick="confirmDelete()" class="flex-1 bg-red-500 text-white py-3 text-xs font-bold tracking-widest hover:bg-red-600 transition-colors">
削除する
