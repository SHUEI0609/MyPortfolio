...
function renderList() {
    ...
    container.innerHTML = items.map((item, index) => {
        ...
        if (currentTab === 'history') {
            ...
            secondaryText = `${item.parentId ? `parent: ${item.parentId} / ` : 'root / '}<span class="text-zinc-500">${item.detail || ''}</span> <span class="text-zinc-300 text-xs">angle ${item.angle || 'auto'} / distance ${item.distance || '42'}</span>`;
        }
